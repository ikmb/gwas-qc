...
def generate_exclude_file_CaseControl(HFresults_file, individuals_annotation_QCed, variant_exclude_file, batches_names, prefix_merged_SNPQCII, FDR_index_remove_variants, plotscript):
    """ determine variants failed even with the worst batch removed """
    ...
    while line:
        ...
        # (4) Cases
        if list[5] != "NA":
            HF_entire_collection_cases = float(list[5])
        else:
    ...
    for j in xrange(len(thresholds_FDR)):
        ...
        for i in xrange(1,n+1,1):
            ...
            if (not break_i_loop_part1) and (HF_Pval_vector_allbatches_ctrls[i-1][0] > threshold):
                ...
                # removeVariants from ctrls/CD/UC allbatches for each FDR
                for k in xrange(i-1):
                    counts_rejected_FDR_allbatches_ctrls_cases[j][HF_Pval_vector_allbatches_ctrls[k][1]] = True
            ...
            if (not break_i_loop_part2) and (HF_Pval_vector_worstbatchremoved_ctrls[i-1][0] > threshold):
                ...
                counts_rejected_FDR_worstbatchremoved_ctrls[j] = i-1
                break_i_loop_part2 = True
                print ("FDR %s Threshold %s > %s\n" %(j, HF_Pval_vector_worstbatchremoved_ctrls[i-1][0], threshold))
                # add variantIDs to dict for counting total number of
                # removeVariants from ctrls/CD/UC worstbatchremoved for each FDR
                for k in xrange(i-1):
                    counts_rejected_FDR_worstbatchremoved_ctrls_cases[j][HF_Pval_vector_worstbatchremoved_ctrls[k][1]] = True

            if break_i_loop_part1 and break_i_loop_part2:
                break

    #print(HF_Pval_vector_worstbatchremoved_ctrls)
    print "Control sample rejections for all batches by FDR: "
    print(counts_rejected_FDR_allbatches_ctrls)
    print "Control sample rejections for all batches but the worst by FDR: "
    print(counts_rejected_FDR_worstbatchremoved_ctrls)
    ...
    # write header
    fh_FDR_w.writelines("FDR\tFail_allbatches_ctrls_cases\tFail_worstbatchremoved_ctrls_cases\tFail_allbatches_ctrls\tHF_pval_allbatches_ctrls\tFail_worstbatchremoved_ctrls\tHF_pval_worstbatchremoved_ctrls\tFail_allbatches_cases\tHF_pval_allbatches_cases\tFail_worstbatchremoved_cases\tHF_pval_worstbatchremoved_cases\n")
